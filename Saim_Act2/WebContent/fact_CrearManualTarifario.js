...
function Radio() {
	...
	for (var x=0; x<radioButtons.length; x++) {
		if (radioButtons[x].checked) {
			...
			ajax.open("POST", "ControlCrearManualTarifario", true);
...
function Ingresar(){
	...
	var codManualB = document.getElementById("listaManualB").value.trim();
	
	if(desc == ""){
		alert("Digite la descripci\xF3n");
		document.getElementById("Descripcion").focus();
		document.getElementById("Descripcion").select();
	}else if(codManualB == "Seleccione"){
		alert("Escoja un manual base");
		document.getElementById("listaManualB").focus();
	}else{
		var ajax = getXMLObject();
		ajax.open("POST", "ControlCrearManualTarifario", true);
		ajax.onreadystatechange = function(){
			if(ajax.readyState == 4){
				if(ajax.responseText == "Ingreso exitoso"){
					...
					document.getElementById("listaManualB").selectedIndex = 0;
					document.getElementById("Descripcion").focus();
				}
		...
		ajax.setRequestHeader('Content-Type','application/x-www-form-urlencoded;charset=utf-8');
		ajax.send("valor=Ingresar&desc="+encodeURIComponent(desc)+"&codManualB="+codManualB);
	}
}
...
function Actualizar(){
	...
	if(desc == ""){
		...
		ajax.onreadystatechange = function(){
			if(ajax.readyState == 4){
				if(ajax.responseText == "Manual tarifario actualizado exitosamente"){
					document.getElementById("Opcion").innerHTML = "";
					var xmlhttp1 = getXMLObject();
					xmlhttp1.open("POST", "ControlCrearManualTarifario", true);
		...
		ajax.setRequestHeader('Content-Type','application/x-www-form-urlencoded;charset=utf-8');
		ajax.send("valor=Actualizar&desc="+encodeURIComponent(desc)+"&codManualB="+codManualB+"&cod="+cod);
	}
}