...
/*función utilizada para realizar la petición asincrona al servidor(AJAX)*/
	...
	function autocompletaMArticulo(idiv,itex,idd) {    
	//	alert(idd);
		iddc=idd;
	                ...
	                if(tecla == 13) { // ENTER o Intro
	                    ...
	                    if(cachesugerencias1[texto] == null) {
	                        ...
	                        peticion.onreadystatechange = function() {
	                               ...
	                               if(idd=="1"){
	                                    peticion.open("POST","ControlExistencias?valor=autoart&texto="+texto,true);                                 
	                               }else{
	...
	function seleccionaElementoArticulo(articulos,idiv, itex) {
	    if(articulos[elementoSeleccionado]) {
		   ...
		   // alert("Selecc iddc: "+iddc+" grupo: "+grupo);
		    if(iddc=="0"){
		    if(grupo==1){
		    	alert("Solo puede modificar Insumos");
		    	document.getElementById("txtTipoMeH"+itex).value = "";
				document.getElementById("txtTipoMe"+itex).value = "";
...
function confirmarArticulo(cf,itex,idiv){		
		...
		// alert("confirmaarticulo iddc: "+iddc+" grupo: "+grupo);
		 if(iddc=="0"){
		 if(grupo==1){
		    	alert("Solo puede modificar Insumos");
		    	document.getElementById("txtTipoMeH"+itex).value = "";
		 ...
		 borraLista1(idiv);
		 document.getElementById('btnMP').disabled = false;//.style.visibility='visible';
