...
/*función utilizada para realizar la petición asincrona al servidor(AJAX)*/
	...
	function autocompletaAnexo3(idiv,itex,xx) { 
		cachesugerencias312 = {};
    	...
    	/////
	        ...
	        if(tecla == 40) { // Flecha Abajo
	            ...
	            muestrasugerencias312(articulos12,idiv,itex);
	        }else 
	            if(tecla == 38) { // Flecha Arriba
	                ...
	                if(tecla == 13) { // ENTER o Intro
	                    seleccionaElementoArticulo32(articulos12, idiv, itex);
	                	//clicksugerencia(choice);
	                }else {
	                	//var texto = txtTipoMe;
	                	//alert("txtTipoMe"+txtTipoMe);
	                    texto = document.getElementById("ref"+itex).value;
	                    // Si es la tecla de borrado y el texto es vacío, ocultar la lista
	                    if(tecla == 8 && texto == "") {
	                        borraLista312(articulos12);
	                        return;
	                    }
	                    if(cachesugerencias312[texto] == null) {
	                        ...
	                        peticion1.onreadystatechange = function() {
	                                        ...
	                                        if(peticion1.status == 200) {	                                        	
	                                        	 articulos12 = eval('('+peticion1.responseText+')');
	                                                if(articulos12.length == 0) {
	                                                    ...
	                                                    actualizasugerencias3112(articulos12,idiv,itex);
	                                                }
	                              ...
	                              // alert("mbase: "+xx);
	                                peticion1.open("POST","ControlProgramas?valor=autocups&texto="+texto+"&xx="+xx,true);                                 
	     	                        peticion1.send("");
	                    } else {
	...
	function sinResultadosArticulo322(idiv) {
		var x="sugerencias321"+idiv;
	    document.getElementById(x).innerHTML = "No existe registro";
	    document.getElementById(x).style.display = "block";
	}

	function actualizasugerencias3112(articulos12,idiv,itex) {
	...
	function muestrasugerencias312(articulos12,idiv,itex) {
		    ...
		    zonasugerencias1.innerHTML = articulos12.formateaLista312(idiv,itex,mostrar);
	...
	function seleccionaElementoArticulo32(articulos12,idiv, itex) {
	    if(articulos12[elementoSeleccionadop]) {
		    ...
		    if(itex=="0"){
		    	document.getElementById("refh").value = codigo;
		    document.getElementById("ref"+itex).value = nombre;
		    document.getElementById("ref1").value = lot;
		    document.getElementById("cant").value = "1";
		    }else{
		    	document.getElementById("refh").value = codigo;
		    document.getElementById("ref"+itex).value = lot;
			document.getElementById("ref0").value = nombre;
			 document.getElementById("cant").value = "1";
		    }
		
    	    borraLista312(idiv);
		   
    	  //  serviciosPYP(lot,ca,nombre,med);
		   
	    }
	}
	function borraLista312(idiv) {
