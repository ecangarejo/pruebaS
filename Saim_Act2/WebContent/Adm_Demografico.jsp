...
//domina el div de la direccion by Greys Hernandez 
 ...
 function limpiar(){
	 ...
	 document.getElementById("txtdiregeorre").value="";
...
//efecto jquery 
/*$(function(){
	 $("#RaMostrar").click(function(event) {
	 event.preventDefault();
	 $("#DirJquery").slideToggle();
	});
...
String fecn=request.getParameter("txtfechanaci");
String gs=request.getParameter("txtgs");
...
System.out.println("nombre "+nombre+" primerap "+primerap+" segundoape "+segundoape);
if(nombre==null){
...
for(i=0;i<primerap.length();i++){
	primerap=primerap.replace('9','ñ');
	primerap=primerap.replace('8','Ñ');
...
for(i=0;i<segundoape.length();i++){
	segundoape=segundoape.replace('9','ñ');
	segundoape=segundoape.replace('8','Ñ');
}
//nom
for(i=0;i<nombre.length();i++){
	nombre=nombre.replace('9','ñ');
	nombre=nombre.replace('8','Ñ');
}
...
<!--  <form id="form2" name="form2" onkeypress = "return pulsar(event);" style="margin-top:-16px;">-->
 ...
 <input type="hidden" name="txturgvital" id="txturgvital" value="<%=id %>" style="visibility:hidden" />
...
<table width="100%" class="style6">
  ...
  <tr>
    ...
    <td width="15%"><span class="Estilo8"> Tipo Documento</span></td>
    <td><label>
...
out.println("no se pudo realizar la conexion!<br/>"); 
  ...
  <tr>
    ...
    <td><label>
      <input name="txtpapellido" type="text" id="txtpapellido" onKeyUp="this.value=this.value.toUpperCase();"  onKeyPress="ajaxEps()" maxlength="20" value="<%=primerap%>" />
      <span class="style8">*</span></label></td>
    <td><span class="Estilo8">Segundo Apellido</span></td>
    <td><label>
      <input name="txtsapellido" type="text" id="txtsapellido" onKeyUp="this.value=this.value.toUpperCase();"  onChange="datos();"  maxlength="20" value="<%=segundoape%>"/>
    </label></td>
  </tr>
  <tr>
    <td><span class="Estilo8">Nombres</span></td>
    <td><label>
      <input name="txtnombre" type="text" id="txtnombre" onKeyUp="this.value=this.value.toUpperCase();"  onChange="" value="<%=nombre%>" />
      <span class="style8">*</span></label> </td>
    <td><span class="Estilo8">Fecha Nacimiento(ddmmaaaa)</span></td>
    <td><label>
      <input type="text" name="txtfechanaci" id="txtfechanaci"   onKeyUp="masca(this,'/',patron,true)" value="<%=fecn %>" maxlength="10"/>
      <span class="style8">*</span></label></td>
  </tr>
  <tr>
...
<!--  <label><input type="radio" name="RaMostrar"  id="RaMostrar" disabled="disabled" /></label> -->
<!-- Muestra la direccion 
<label>
<input name="txtdire" type="text" id="txtdire"  style="border:0px" readonly maxlength="100" class="simpletext">
</label>-->
...
<td colspan="2">
 <table>
  ...
  <tr>
    ...
    <td><input name="txtbarrio" type="text" id="txtbarrio" onKeyUp="this.value=this.value.toUpperCase();"  maxlength="20"/>
<span class="style8">*</span></label></td>
  ...
  <tr>
    ...
    <td><label>
      <input name="txtraza" type="text" id="txtraza" onKeyUp="this.value=this.value.toUpperCase();" onChange="datos();"  maxlength="20"/>
    </label></td>
  ...
  <tr>
    <td>&nbsp;</td>
    <td><label>
      <input name="txtcodentidad" type="hidden" id="txtcodentidad" value="<%=cod%>" />
      <input name="txtcodmun" type="hidden" id="txtcodmun" />
      <input name="txtocupacion" type="hidden" id="txtocupacion"  style="visibility:hidden" />
      <input name="txtnomusu" type="hidden" id="txtnomusu"  value="<%=nomUsu %>" style="visibility:hidden" >
      <input name="txtcodcama" type="hidden" id="txtcodcama"  value="<%=codcam %>" />
      <input name="txtCodColaFin" type="hidden" id="txtCodColaFin"  value="<%=CodColaFin %>" />


        <!--datos de longitud y latitud de la direccion actual  -->

        <input name="txtlongitud" type="hidden" id="txtlongitud" /> <!-- longitud -->
        <input name="txtlatitud"  type="hidden" id="txtlatitud"  /> <!-- latitud -->
        <input name="txtdiregeorre" type="hidden" id="txtdiregeorre" readonly /><!-- Capo oculto pa la georreferenciacion -->


    </label></td>
    <td>
   <label>
      <input type="button" name="btingresar" class="boton4" id="btingresar" value="     Ingresar    "  onClick="demografico(form2)"/>
   </label>
    </td>
    </tr>
