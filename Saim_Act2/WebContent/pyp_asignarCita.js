...
function CargarProgramas(){
	...
	ajax.setRequestHeader('Content-Type','application/x-www-form-urlencoded;charset=utf-8');
	ajax.send("valor=BPRO&Programa="+Programa+"&CodManual="+CodManual);
	}
		...
		if(CodigoPrograma==""){
			...
			if(TipoOrden=="Seleccione"){
				...
				ajax.send("valor=AOS&CodigoPrograma="+CodigoPrograma+"&CodCups="+CodCups+"&CodReferencia="+CodReferencia+
						"&CodEnt="+CodEnt+"&CodPac="+CodPac+"&Servicio="+Servicio+"&ValorPrograma="+ValorPrograma+
						"&NumOrden="+NumOrden+"&Observacion="+Observacion+"&Cantidad="+Cantidad+"&CodOrden="+CodOrden+
						"&Codusuario="+Codusuario+"&ConsecutivoOrden="+ConsecutivoOrden+"&NomEntidad="+NomEntidad+"&DireEntiPrestadora="+DireEntiPrestadora+
	...
	if(opcion){
		...
		ajax.onreadystatechange = function(){
			if(ajax.readyState == 4){
				...
				ReporteOrdenServicio(ajax.responseText)
...
function BuscarProgCex(){
	...
	ajax.setRequestHeader('Content-Type','application/x-www-form-urlencoded;charset=utf-8');
	ajax.send("valor=BPDX&Programa="+Programa+"&ManualTarifario="+ManualTarifario);
}
...
function ReporteRecordatorioCita(CodHoraMedico) {		
	pagina="pyp_ReporteRecordatorioCita.jsp?CodHoraMedico="+CodHoraMedico;			
   	var opciones="toolbar=no, location=no, directories=no, status=no, menubar=no,";
 	opciones =opciones+"scrollbars=si, resizable=yes, width=800, height=600, top=85,  left=140";
   	window.open(pagina,"NUEVA",opciones);		
}
