...
function Radio() {
	...
	for (var x=0; x<radioButtons.length; x++) {
		if (radioButtons[x].checked) {
			...
			ajax.open("POST", "ControlCrearRIPS", true);
			ajax.onreadystatechange = function(){
				if (ajax.readyState == 4) {
					if(ajax.status == 200) {
						contenido.innerHTML = ajax.responseText;
					}else{
						alert("EErrores during AJAX call. Please try again");
					}
				}
			};
...
function Ingresar(){
	...
	if(desc == ""){
		...
		document.getElementById("Sigla").select();
	}else{
		var ajax = getXMLObject();
		ajax.open("POST", "ControlCrearRIPS", true);
		ajax.onreadystatechange = function(){
		...
		ajax.setRequestHeader('Content-Type','application/x-www-form-urlencoded;charset=utf-8');
		ajax.send("valor=Ingresar&desc="+encodeURIComponent(desc)+"&sigla="+sigla);
	}
}
...
function Actualizar(){
	...
	if(desc == ""){
		...
		ajax.onreadystatechange = function(){
			if(ajax.readyState == 4){
				if(ajax.responseText == "RIPS actualizado exitosamente"){
					document.getElementById("Opcion").innerHTML = "";
					var xmlhttp1 = getXMLObject();
					xmlhttp1.open("POST", "ControlCrearRIPS", true);
		...
		ajax.setRequestHeader('Content-Type','application/x-www-form-urlencoded;charset=utf-8');
		ajax.send("valor=Actualizar&desc="+encodeURIComponent(desc)+"&sigla="+sigla+"&cod="+cod);
	}
}