...
function informes_ecotrans_Por_Paciente() {
	...
	if (NumeroDocumento == "") {
		...
		ajax = getXMLObject();
		ajax.open("POST", "ControlCrearTransesofagico", true); // getname will be the servlet name
		ajax.onreadystatechange = function() {
			if (ajax.readyState == 4) {
			document.getElementById('informes_ecotransesofagico_paciente').innerHTML = ajax.responseText;
			}
		};
