...
function Radio() {	
	...
	for (var x = 0; x < radioButtons.length; x ++) {
		if (radioButtons[x].checked) {
			...
			ajax.open("POST","ControlCrearArea",true); //getname will be the servlet name
...
function formulario(){
	 if(xmlhttp) { 	
		    xmlhttp.open("POST","ControlCrearArea?valor=0",true); //getname will be the servlet name
...
function VerPreguntas(){
	...
	if((CodigoArea!="Seleccione")){
		ajax=getXMLObject();
		ajax.open("POST","ControlCrearArea",true);
		ajax.onreadystatechange = function(){
			if(ajax.readyState == 4){
...
function AsignarPregunta(){
	...
	if((NomArea!="")&&(CodPregunta!="")){
	 if(xmlhttp) { 	
		    xmlhttp.open("POST","ControlCrearArea?valor=1&NomArea="+NomArea+"&CodPregunta="+CodPregunta,true); //getname will be the servlet name
...
function regresar(){
	var NomArea=document.getElementById("txtNomArea").value;
	 if(xmlhttp) { 		 
		    xmlhttp.open("POST","ControlCrearArea?valor=4&NomArea="+NomArea,true); //getname will be the servlet name
		    xmlhttp.onreadystatechange  = ComprobarAsignarPregunta;
		    xmlhttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded;charset=utf-8');
		    xmlhttp.send(""); //Posting txtname to Servlet
		   }
...
function omitir(codRelacion){
	...
	xmlhttp.open("POST","ControlCrearArea?valor=3&codRelacion="+codRelacion,true); //getname will be the servlet name
