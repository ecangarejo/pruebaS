...
function formulario(){
	 if(xmlhttp) { 	
		    xmlhttp.open("POST","ControlCrearPregunta?valor=0",true); //getname will be the servlet name
	   ...
	   if (xmlhttp.readyState == 4) {
		if(xmlhttp.status == 200) {
			...
			if(aviso!=""){
				...
				window.location.href="hic_crearPregunta.jsp";
			}
	     }
	     else {
	        alert("Error during AJAX call. Please try again");
...
function CrearCerrada(TipoRequerimiento){
	...
	var NombrePregunta=encodeURIComponent(document.getElementById("txtNomPregunta").value);
	...
	if((CodTipoRespuesta!="")&&(NombrePregunta!="")){
	 if(xmlhttp) { 	
		    xmlhttp.open("POST","ControlCrearPregunta?valor=1&CodTipoRespuesta="+CodTipoRespuesta+"&NombrePregunta="+NombrePregunta+"&PatronNormal="+PatronNormal+"&UnidadPregunta="+UnidadPregunta+"&TipoRequerimiento="+TipoRequerimiento,true); //getname will be the servlet name
