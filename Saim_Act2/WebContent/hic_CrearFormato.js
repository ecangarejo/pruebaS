...
function ActualizarFormato(){
	...
	if((CodigoFormato!="Seleccione")&&(CodigoArea!="Seleccione")){
		ajax=getXMLObject();
		ajax.open("POST","ControlFormatoHic",true);
		ajax.onreadystatechange = function(){
			if(ajax.readyState == 4){
...
function formulario(){
	 if(xmlhttp) { 	
		    xmlhttp.open("POST","ControlFormatoHic?valor=0",true); //getname will be the servlet name
...
function AsignarArea(){
	...
	if((NomFormato!="")&&(CodArea!="")){
	 if(xmlhttp) { 	
		    xmlhttp.open("POST","ControlFormatoHic?valor=1&NomFormato="+NomFormato+"&CodArea="+CodArea,true); //getname will be the servlet name
...
function regresar(){
	var NomFormato=document.getElementById("txtNomFormato").value;
	 if(xmlhttp) { 		 
		    xmlhttp.open("POST","ControlFormatoHic?valor=4&NomFormato="+NomFormato,true); //getname will be the servlet name
		    xmlhttp.onreadystatechange  = ComprobarAsignarArea;
...
function omitir(codRelacion){
	...
	xmlhttp.open("POST","ControlFormatoHic?valor=3&codRelacion="+codRelacion,true); //getname will be the servlet name
