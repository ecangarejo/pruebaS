...
function informes_pyp_planfam_por_paciente() {
	...
	if (NumeroDocumento == "") {
		...
		ajax.send("valor=Modificar&TipoDocumento=" + TipoDocumento
...
function modificarInformePYPPlanFam(idcodInforme, idCodigopaciente){
	pagina="pyp_Modificacion_Pyp_PlanFam.jsp?CodInfPYPPlanFam="+idcodInforme+"&codPaciente="+idCodigopaciente;			
   	var opciones="toolbar=no, location=no, directories=no, status=no, menubar=no,";
 	opciones =opciones+"scrollbars=yes, resizable=yes, width=800, height=600, top=85, left=140";
   	window.open(pagina,"Modificacion_De_PYP_PlanFam",opciones);
}
