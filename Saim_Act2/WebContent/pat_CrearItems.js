		 ...
		 var radioButtons = document.getElementsByName("radiobutton");
	      for (var x = 0; x < radioButtons.length; x ++) {
	        if (radioButtons[x].checked) {
		         ...
		         xmlhttp.open("POST","Pat_ControlCrearFormato?valor="+val,true); //getname will be the servlet name
...
function CrearTexto(){
	...
	if(pregunta!=""){
	ajax.open("POST","Pat_ControlCrearFormato",true); //getname will be the servlet name
...
function formulario(){
	 if(xmlhttp) {
		    xmlhttp.open("POST","Pat_ControlCrearFormato?valor=0",true); //getname will be the servlet name
...
function buscardatos(){
	var pregunta=document.getElementById("txtArea").value;
	if(xmlhttp) {
		xmlhttp.open("POST","Pat_ControlCrearFormato?valor=3&pregunta="+pregunta+"&tipo=2",true); //getname will be the servlet name
	    xmlhttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
	    xmlhttp.send(""); //Posting txtname to Servlet
	}
...
function GuardarSeleccion(respuesta){
	...
	if(xmlhttp) { 
		var pregunta=document.getElementById("txtArea").value;
	    xmlhttp.open("POST","Pat_ControlCrearFormato?valor=4&respuesta="+respuesta+"&pregunta="+pregunta,true); //getname will be the servlet name
	    xmlhttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
	    xmlhttp.send(""); //Posting txtname to Servlet	
	}
	}
