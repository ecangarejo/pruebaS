...
function dibujar_ventriculografia() {
	...
	// alert(arreglodibujorealizadosventriculografia.toString());
	ctx.fillStyle = "red";
	ctx.strokeStyle="red";
	ctx.lineWidth=2; 
...
function reiniciar_Elementos_Formulario_Anatomia_Coronaria() {
	...
	var j=0;
		...
		while(j < numeroItemsCombobox("Seleccion_Arteria_combo")){
...
function identificaciondearteriaporclic(idlesion){
	...
	var cadenalesionesarteria = "";
	for (var i = 0; i < tipo_lesiones_arterias_general.length ; i++){
	
		cadenalesionesarteria = tipo_lesiones_arterias_general[i].split("-");
		//alert(cadenalesionesarteria.toString() +" "+cadenalesionesarteria.length)
		arteriaseleccion = 0;
...
function identificacion_areas_seleccionadas(posx, posy) {
	...
	//alert(i +"aresa seleccionada "+posx +" "+posy );
    ...
    if (tamanoinfotextbox('resumen_procedimiento')>0){
		if(tamanoinfotextbox('conclusiones')>0){
			if(tamanoinfotextbox('recomendaciones')>0){
				desactivarelementoFormulario('guardar');
				desactivarelementoFormulario('anular');
				ajax = getXMLObject();
					ajax.open("POST", "cateterismodata", true);
					ajax.onreadystatechange = function() {
						if (ajax.readyState == 4) {
							if (ajax.responseText != '0') {
								alert("ha realizado correctamente el informe");
