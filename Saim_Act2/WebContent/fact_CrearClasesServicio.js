...
function Radio() {
	...
	for (var x=0; x<radioButtons.length; x++) {
		if (radioButtons[x].checked) {
			...
			ajax.open("POST", "ControlClasesServicio", true);
...
function Actualizar(){
	...
	if(desc == ""){
		...
		ajax.onreadystatechange = function(){
			if(ajax.readyState == 4){
				if(ajax.responseText == "Clase de servicio actualizada exitosamente"){
					document.getElementById("Opcion").innerHTML = "";
					var xmlhttp = getXMLObject();
					xmlhttp.open("POST", "ControlClasesServicio", true);
					...
					xmlhttp.send("valor=ActualizarRb");
				}
				alert(ajax.responseText);
			}
		}
