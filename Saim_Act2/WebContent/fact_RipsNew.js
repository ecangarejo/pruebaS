...
function BuscarF() {
	...
	if (cmbTipoRips == "BusGen") {
		...
		if ((fi == "") || (ff == "")) {
			...
			ajax.setRequestHeader('Content-Type','application/x-www-form-urlencoded;charset=utf-8');
			if(eps == "Seleccione"){
				ajax.send("valor=3.1&fi=" + fi + "&ff=" + ff); // Posting txtname to Servle
			}else{
...
function Exportar() {
	...
	//var rut = "//usr//share//tomcat6//webapps//Saim";
	if (nom == "") {
...
function CT(rut,nom, fi, ff, fsl, datos, CodEnt, ctac, ctaf, ctam, ctus, ctat,ctau, ctap, ctrn, ctah) {
	...
	ajax.onreadystatechange = function() {
		...
		if (ajax.readyState == 4) {
			...
			zip(rut,nom, ctac, ctaf, ctam, ctus, ctat, ctau, ctap, ctrn, ctah);
		}
	}
	ajax.setRequestHeader('Content-Type','application/x-www-form-urlencoded;charset=utf-8');
	ajax.send("valor=5&rut=" + rut +"&nom=" + nom + "&fi=" + fi + "&ff=" + ff + "&fsl=" + fsl+ "&datos=" + datos + "&CodEnt=" + CodEnt + "&ctac=" + ctac
...
function zip(rut,nom, ctac, ctaf, ctam, ctus, ctat, ctau, ctap, ctrn, ctah) {
	document.getElementById('form1').innerHTML = '<p>Generando Archivos RIPS... No Cierre La Ventana...</p><img src="Imagenes/Noventa.gif"> 99%';
