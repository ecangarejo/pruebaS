...
function Radio() {
	...
	for (var x=0; x<radioButtons.length; x++) {
		if (radioButtons[x].checked) {
			...
			ajax.open("POST", "ControlCrearNivelComplejidad", true);
...
function Actualizar(){
	...
	if(desc == ""){
		...
		ajax.onreadystatechange = function(){
			if(ajax.readyState == 4){
				if(ajax.responseText == "Nivel de complejidad actualizado exitosamente"){
					document.getElementById("Opcion").innerHTML = "";
					var xmlhttp1 = getXMLObject();
					xmlhttp1.open("POST", "ControlCrearNivelComplejidad", true);
