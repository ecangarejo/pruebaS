...
function BuscarReciboCaja(){
	var NumReciboCaja=document.getElementById("txtNumReciboCaja").value;
	 ajax=getXMLObject();
		ajax.open("POST","ControlEspecialidades",true); //getname will be the servlet name
...
function GuardarEgresos(){
	...
	var Codusuario=document.getElementById("txtCodusuario").value;
	 ajax=getXMLObject();
		...
		ajax.onreadystatechange  = function(){
			if(ajax.readyState == 4){
				window.location.href("agm_recibos_egreso.jsp");
...
function VerOtro(){
	...
	ajax.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded;charset=utf-8');
	ajax.send("valor=rc3.1&Concepto="+Concepto+"&CodPac="+CodPac);
}
/////////////////////////////////////////////////////////////////////////////
function BuscarPaciente(){
	...
	ajax=getXMLObject();
	ajax.open("POST","ControlEspecialidades",true);
	ajax.onreadystatechange = function(){
		if(ajax.readyState == 4){
	...
	if((ValorOrden!="")&&(Abono=="")){
		...
		ValoraRecibir=ValorOrden;
				...
				ajax=getXMLObject();
					...
					ajax.onreadystatechange  = function(){
						if(ajax.readyState == 4){
							window.location.href("agm_recibos_de_caja_actualizar.jsp");
							document.location.href = document.location.href;
...
function GuardarReciboCaja(){
	document.getElementById("btnGuardarRc").disabled=true;
	var TipoPago="";
	var Abono=document.getElementById("txtAbono").value;
	...
	var NumOrden=document.getElementById("txtNumOrden").value="-";
	//var TotalPagar=document.getElementById("txtTotalPagar").value;
	var TotalPagar=document.getElementById("txtAbono").value;
	var Observacion=document.getElementById("txtObservacion").value;
	var radioButtons = document.getElementsByName("radiobutton");
		...
		if(TipoPago==" "){
			...
			if(ValCmb=="Seleccione"){
				...
				if((ValorOrden!="")&&(NumOrden=="")){
					...
					if(Otro==""){
						alert("Escriba la Admision ha Afectar.");
						document.getElementById("btnGuardarRc").disabled=false;
					}else{
						...
						ajax.onreadystatechange  = function(){
							if(ajax.readyState == 4){
								window.location.href("agm_recibos_de_caja.jsp");
								document.location.href = document.location.href;
								CodDetRecCaj = ajax.responseText;
								ImprimirReciboCaja(CodDetRecCaj)
							}
						...
						ajax.send("valor=rc2&Abono="+Abono+"&ValoraRecibir="+ValoraRecibir+"&ValorRecibido="+ValorRecibido+
								...
								"&CuotaModeradora="+CuotaModeradora+"&ValorOrden="+ValorOrden+"&NumOrden="+NumOrden+
								"&TotalPagar="+TotalPagar+"&cheqconsig="+cheqconsig+"&Observacion="+Observacion+"&CodAdmision="+Otro+"&TipoConcepto="+Conceptos);
					}
				}
			}
		}		
