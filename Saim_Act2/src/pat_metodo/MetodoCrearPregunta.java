		 ...
		 CrearPregunta cp = new CrearPregunta();
		   ...
		   cp.setTipoRequerimiento(TipoRequerimiento);
			try{
				    ...
				    ps=con.conn.prepareStatement("insert into pat_pregunta(nombre_pregunta,patronNormal,unidad,requerido)values(?,?,?,?)");
		        	...
		        	System.out.println("ERROR EN MetodoCrearPregunta>>CrearPregunta "+ex);
				}
			}
		 ...
		 */
		 ...
		 cp.setcodigo_pregunta_fk(codigo_pregunta_fk);
			try{
					PreparedStatement ps = null;
				    Conexion con=new Conexion();
				    ps=con.conn.prepareStatement("insert into pat_pregunta_tiporespuesta(codigo_pregunta_fk,codigo_tiporespuesta_fk) values (?,?)");
		        	...
		        	System.out.println("ERROR EN MetodoCrearPregunta>>RelacionPreguntaTipoRespuesta "+ex);
				}
		 ...
		 * */
		  java.sql.ResultSet r=null;
	       ...
	       Conexion con=new Conexion();
	        	...
	        	r=st.executeQuery("SELECT codigo,nombre_tiporespuesta FROM pat_tipo_respuesta WHERE nombre_tiporespuesta LIKE '"+cod+"%'");
	        return r;
	    }
	...
	public ResultSet ObtenerCodigoPregunta(String nombrePregunta) throws Exception {
		 ...
		 */
		  java.sql.ResultSet r=null;
	       ...
	       Conexion con=new Conexion();
	        	st = con.conn.createStatement();
	        	r=st.executeQuery("select codigo,nombre_pregunta from pat_pregunta where nombre_pregunta='"+nombrePregunta+"'");
	        return r;
	    }
}
