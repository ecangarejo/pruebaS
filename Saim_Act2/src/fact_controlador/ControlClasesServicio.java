...
import fact_metodo.MetodoCrearClasesServicio;

/**
 * Servlet implementation class ControlClasesServicio
 */
public class ControlClasesServicio extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		MetodoCrearClasesServicio mce = new MetodoCrearClasesServicio();
		...
		if(va.equals("CrearRb")){
			...
			out.print("<tr><td width='166'><div align='right'>Seleccione la Clase de Servicio:</div></td><td width='318'><select name='lista' id='lista' onchange='Buscar()'><option value='Seleccione'>Seleccione</option>");
			try {
				while(rs.next()){
			...
			try {
				if(rs1.next()){
					...
					out.print("<tr><td colspan='2'><div align='right'><input name='Actualizar' type='button' id='Actualizar' value='   Actualizar   ' onclick='Actualizar()' /></div></td></tr></table>");
				}
			...
			if(mce.Actualizar(desc, cod)){
				out.print("Clase de servicio actualizada exitosamente");
			}else{
				out.print("Error al actualizar la clase de servicio");
			}
			out.close();
		}
	}
