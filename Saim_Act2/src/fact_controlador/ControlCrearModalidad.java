...
/**
 * Servlet implementation class ControlCrearModalidad
 */
public class ControlCrearModalidad extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		MetodoCrearModalidad mce = new MetodoCrearModalidad();
		...
		if(va.equals("CrearRb")){
			...
			out.print("<tr><td width='206'><div align='right'>Seleccione la Modalidad:</div></td><td width='318'><select name='lista' id='lista' onchange='Buscar()'><option value='Seleccione'>Seleccione</option>");
			try {
				while(rs.next()){
			...
			if(mce.Actualizar(desc, cod)){
				out.print("Modalidad actualizada exitosamente");
			}else{
				out.print("Error al actualizar la modalidad");
			}
			out.close();
		}
	}
