...
/**
 * Servlet implementation class ControlCrearManualBase
 */
public class ControlCrearManualBase extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		MetodoCrearManualBase mce = new MetodoCrearManualBase();
		...
		if(va.equals("CrearRb")){
			...
			out.print("<tr><td width='206'><div align='right'>Seleccione el Manual Base:</div></td><td width='318'><select name='lista' id='lista' onchange='Buscar()'><option value='Seleccione'>Seleccione</option>");
			try {
				while(rs.next()){
			...
			if(mce.Actualizar(desc, cod)){
				out.print("Manual base actualizado exitosamente");
			}else{
				out.print("Error al actualizar el manual base");
			}
			out.close();
		}
	}
