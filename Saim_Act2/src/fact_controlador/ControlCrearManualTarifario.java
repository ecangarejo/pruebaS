...
import fact_metodo.MetodoCrearManualBase;
import fact_metodo.MetodoCrearManualTarifario;

/**
 * Servlet implementation class ControlCrearManualTarifario
 */
public class ControlCrearManualTarifario extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		MetodoCrearManualTarifario mce = new MetodoCrearManualTarifario();
		MetodoCrearManualBase mb = new MetodoCrearManualBase();
		
		String desc = req.getParameter("desc");
		String codManualB = req.getParameter("codManualB"); 
		...
		if(va.equals("CrearRb")){
			...
			out.print("<tr><td><div align='right'>Manual Base:</div></td><td><select name='listaManualB' id='listaManualB'><option value='Seleccione'>Seleccione</option>");
			rs3 = mb.getAll();
			try {
				while (rs3.next()) {
			...
			out.print("<tr><td width='206'><div align='right'>Seleccione el Manual Tarifario:</div></td><td width='318'><select name='lista' id='lista' onchange='Buscar()'><option value='Seleccione'>Seleccione</option>");
			try {
				while(rs.next()){
			...
			try {
				if(rs1.next()){
					...
					try {
						while (rs2.next()) {
							if(rs1.getString(2).equals(rs2.getString(1))){
								out.print("<option value='"+rs2.getString(1)+"' selected='selected'>"+rs2.getString(2)+"</option>");
							}else{
			...
			if(mce.Actualizar(desc, codManualB, cod)){
				out.print("Manual tarifario actualizado exitosamente");
			}else{
				out.print("Error al actualizar el manual tarifario");
			}
			out.close();
		}
	}
