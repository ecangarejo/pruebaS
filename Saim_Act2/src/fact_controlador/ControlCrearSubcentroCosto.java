...
import fact_metodo.MetodoCrearSubcentroCosto;

/**
 * Servlet implementation class ControlCrearSubcentroCosto
 */
public class ControlCrearSubcentroCosto extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		MetodoCrearSubcentroCosto mce = new MetodoCrearSubcentroCosto();
		...
		if(va.equals("CrearRb")){
			...
			out.print("<tr><td width='206'><div align='right'>Seleccione el Subcentro de Costo:</div></td><td width='318'><select name='lista' id='lista' onchange='Buscar()'><option value='Seleccione'>Seleccione</option>");
			try {
				while(rs.next()){
			...
			if(mce.Actualizar(desc, cod)){
				out.print("Subcentro de costo actualizado exitosamente");
			}else{
				out.print("Error al actualizar el subcentro de costo");
			}
			out.close();
		}
	}
