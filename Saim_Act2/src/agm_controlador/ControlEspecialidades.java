 ...
 */
public class ControlEspecialidades extends HttpServlet {
...
/***********************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************/
						...
						//out.print("<tr><td width='12%' class='style12'>Ciudad y Fecha </td><td width='41%'>"+rs2.getString(1)+",  "+rs2.getString(4)+" de "+mes+" "+rs2.getString(2)+"</td><input type=hidden id='txtFechaLetra' value='"+FechaLetras+"' /><td class='style12' colspan='2'>Valor Servicio:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$<input type='text' name='txtTotalPagar' id='txtTotalPagar' /></td><td class='style12'>Valor a Recibir $<input name='txtAbono' type='text' id='txtAbono'  onkeyup='Letras()' /></td></tr>");
							out.print("<tr><td width='12%' class='style12'>Ciudad y Fecha </td><td width='41%'>"+rs2.getString(1)+",  "+rs2.getString(4)+" de "+mes+" "+rs2.getString(2)+"</td><input type=hidden id='txtFechaLetra' value='"+FechaLetras+"' /><td class='style12' colspan='2'><input type='hidden' name='txtTotalPagar' id='txtTotalPagar' />Valor a Recibir $<input name='txtAbono' type='text' id='txtAbono'  onkeyup='Letras()' /></td><td class='style12'></td></tr>");
						}
						rs2.getStatement().getConnection().close();
						...
						/***************************************************************************************/
						out.print("<table width='100%' border='1'><tr><td width='50%'>");
						out.print("<table width='100%' border='1'><tr><td align='center' colspan='3' >HISTORIAL DE RECIBOS</td></tr></table>");
						//String CodRecCaj=req.getParameter("CodRecCaj");
						rs=mac.AbonosReciboCaja(CodPac1);
					...
					out.print("Paciente Con la Identificacion "+TipoDocumento+"-"+NumeroDocumento+" No Existe.");					
				}					
					rs.getStatement().getConnection().close();
			}catch (SQLException e) {
				e.printStackTrace();
...
if(va.equals("rc2")){
			...
			String TipoConcepto=req.getParameter("TipoConcepto");
			//System.out.println("CuotaModeradora "+CuotaModeradora+" ValorOrden "+ValorOrden+" CantAbono "+CantAbono);
			int valple=0;
			try {
				if(CodReciboCaja.equals("")){
					if(CodAdmision.equals(".")){
						System.out.println("NO ACTUALIZA ADMISION.");
						}else{
							rs1=mac.BuscarCopagoAnticipos(CodAdmision);
							int Copago=0;
							int Anticipos=0;
							if(rs1.next()){
								Copago=rs1.getInt("copago")+Integer.parseInt(CantAbono);
								Anticipos=rs1.getInt("anticipos")+Integer.parseInt(CantAbono);
							}
							rs1.getStatement().getConnection().close();
						//System.out.println("ACTUALIZAR EL ENCABEZADO CON LA ADMISION="+CodAdmision);
						if((TipoConcepto.equals("COPAGO"))||(TipoConcepto.equals("CUOTA MODERADORA"))){
							// se actualiza el campo de copago dela admision digitada
							mac.ActualizarCopago(CodAdmision, Copago+"");
						}
						if(TipoConcepto.equals("ANTICIPO")){
							// se actualiza el campo de anticipo de la admision digitada
							//CantAbono
							mac.ActualizarAnticipo(CodAdmision, Anticipos+"");
						}
					}
...
//			if((Concepto.equals("11"))||(Concepto.equals("12"))||(Concepto.equals("10"))){
			if((Concepto.equals("1"))||(Concepto.equals("2"))||(Concepto.equals("12"))){
				out.print("Aplica a Admision N� <input type=text id='txtOtro' />");
			}else{
				out.print("<input type=hidden id='txtOtro' value='.' />");
			}
		}
